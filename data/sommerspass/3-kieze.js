...
$.getJSON("maps/kieze.geojson", function(data) {
		// Maximum aus dem Array der Kiezflächengrößen herausfinden
		var arr = data.features;

		for (var i=0 ; i<arr.length ; i++) {
			if (arr[i].properties["GROESSE_M2"] > maxGroesse){
				maxGroesse = arr[i].properties["GROESSE_M2"];
			}
		};
...
function funktionenKieze(feature, layer){
	...
	var kiezName = feature.properties["PLR_NAME"];
	var kiezGroesse = feature.properties["GROESSE_M2"].toLocaleString("de-DE", {maximumFractionDigits: 0});
	var content = 	"<b>"
					+ kiezName 
					+ "</b><br>"
					+ "Größe: " 
	...
	layer.setStyle({
		fillOpacity: feature.properties["GROESSE_M2"] / maxGroesse,
	});
